...
function initializ() {
  ...
  if (!chrome.proxy || !chrome.proxy.settings) {
    console.warn("Proxy API недоступен при инициализации");
    return;
  }
  ...
  chrome.proxy.settings.get(
    ...
    function (
      ...
      if (chrome.runtime.lastError) {
        console.error("Ошибка получения настроек прокси:", chrome.runtime.lastError.message);
        return;
      }
...
function toggl() {
  ...
  if (!chrome.proxy || !chrome.proxy.settings) {
    ...
    var androidMsg = chrome.i18n.getMessage("android_warning");
    if (!androidMsg || androidMsg === "") {
      androidMsg = "⚠️ Внимание: Proxy API недоступен.\n\nНа Android (Kiwi Browser) Proxy API может не работать из-за ограничений системы.\n\nРасширение попытается использовать альтернативный метод, но функциональность может быть ограничена.\n\nРекомендуется использовать расширение на десктопной версии Chrome/Edge для полной функциональности.";
    }
    alert(androidMsg);
    return;
  }
          ...
          .then(() => {
            ...
            if (chrome.tabs && chrome.tabs.create) {
              chrome.tabs.create({ url: "https://www.youtube.com/" }, (tab) => {
                if (chrome.runtime.lastError) {
                  console.error("Ошибка открытия YouTube:", chrome.runtime.lastError.message);
                }
              });
            } else {
              console.warn("chrome.tabs API недоступен");
            }
